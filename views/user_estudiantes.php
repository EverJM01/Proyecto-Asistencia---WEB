...
<div class="container-fluid px-4">
    <div class="ms-4 mt-4 mb-4">
        <h1><i class="fa-solid fa-graduation-cap"></i> Configurar Estudiantes</h1>
    </div>
    
    <div class="accordion mt-4 mb-4"  id="accorReEst">
        <div class="accordion-item ">
            <div class="accordion-header" id="headingOne">
                <button class="btn btn-outline-success w-100 py-0" type="button" data-bs-toggle="collapse" data-bs-target="#collapseFormCrearEst" aria-expanded="true" aria-controls="collapseOne">
                    <h5 class="pt-1"><i class="fa-regular fa-square-plus"></i> Registrar Nuevo Estudiante</h5>
                </button>
            </div>
            <div id="collapseFormCrearEst" class="accordion-collapse collapse" aria-labelledby="headingOne" data-bs-parent="#accordionExample">
                <div class="accordion-body">
                    ...
                    <div class="d-flex justify-content-center">
                        <button type="submit" class="btn btn-success rounded-pill mx-1 px-5"><i class="fa-solid fa-circle-plus"></i>  Crear Estudiante</button>
    ...
    <?php
    $aux = 0;
    $queryDoc = "SELECT e.id, e.dni, e.nombre, e.apellido, e.direccion, e.fechanac, e.foto, e.genero, a.nombre, e.id_aula  from estudiante e inner join aula a on a.id = e.id_aula";
    if(isset($_GET["dni_f"]) || isset($_GET["aula_f"]) || isset($_GET["gen_f"])){
        $queryDoc .= " WHERE ";
        
        if(isset($_GET["dni_f"])){
            $aux = 1; 
            $queryDoc .= " e.dni = '".$_GET["dni_f"]."' ";   
        }
        if(isset($_GET["aula_f"])){
            if($aux == 1){
                $queryDoc .= " AND  e.id_aula = '".$_GET["aula_f"]."' "; 
            }else{
                $queryDoc .= " e.id_aula = '".$_GET["aula_f"]."' "; 
            }
            $aux = 2;
        }
        if(isset($_GET["gen_f"])){
            if($aux == 2 || $aux == 1){
                $queryDoc .= " AND e.genero = '".$_GET["gen_f"]."' ";  
            }else{
                $queryDoc .= " e.genero = '".$_GET["gen_f"]."' ";  
            }
        }
    }
            ...
            <table class="table  table-hover" id="tablaDocente">
                ...
                <tbody>
                    <?php
                        ...
                        $sqldoce_l->bind_result($id_doc_l, $cod_doc_l, $nom_doc_l, $ape_doc_l, $dir_doc_l, $nac_doc_l,  $foto_doc_l,  $gen_doc_l,  $aula_doc_l, $aulaid_est);
                        $sqldoce_l->execute();
                        ...
                        while($sqldoce_l->fetch()){
                            ...
                            <tr>
                                ...
                                <td> 
                                    ...
                                    onclick="editarDocLista(<?=$id_doc_l?>, '<?=$cod_doc_l?>', '<?=$nom_doc_l?>' , '<?=$ape_doc_l?>', '<?=$dir_doc_l?>', '<?=$nac_doc_l?>',  '<?=$gen_doc_l?>', '<?=$aula_doc_l?>', <?=$aulaid_est?>)">
        ...
        <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <i class="fa-solid fa-sliders"></i> Editar Estudiante
                    </h3>
                ...
                <div class="modal-body ">
                    <form id="formEditarDocAdmin">
                        <div class="d-flex justify-content-center">
                            ...
                            <button type="button" class="btn btn-danger rounded-pill w-50 mx-1"  name="btnBorrarDoc" id="btn_doc_delete"><i class="fa-regular fa-trash-can"></i> Borrar Estudiante</button>
                        </div>
...
<script src="/app_asistencia/src/js/estudiantes.js"></script>
